...
[[glossary-composer]] Composer::
   The PHP dependency manager used by Drupal, <<glossary-drush, Drush>>, the
   Symfony framework and others. It is the preferred means of installing Drupal
   projects. See <<install-composer>> for more information.
...
[[glossary-drush]] Drush::
  Command line shell and scripting interface for Drupal. See <<install-tools>>
  for more information.
