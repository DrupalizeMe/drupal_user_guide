...
[role="summary"]
Overview of development sites with an example deployment workflow for site
building.
...
Local environment::
  ...
  fixes, theming, and configuration in their local environment. The recommended
  tool for setting up a local environment is DDEV. See <<install-ddev>>.
