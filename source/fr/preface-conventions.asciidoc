...
* Ce guide couvre la version majeure en cours du coœur du logiciel.
...
d'administration, et chaque fois que c'est possible, en utilisant la dernière
version stable de l'outil en ligne de commande Drush (consulter
<<install-tools>>). Le lecteur est libre de se contenter de l'interface
d'administration, selon sa préférence.
