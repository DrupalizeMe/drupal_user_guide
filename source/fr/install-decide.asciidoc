...
Utiliser Composer::
  Si vous prévoyez de construire un site, et de continuer à le développer au
  cours du temps, vous devriez utiliser Composer pour télécharger le cœur, parce
  que Composer est la manière la plus convenable de gérer les dépendances. Si
  vous prévoyez d'utiliser Drush (consulter <<install-tools>>), l'utilisation de
  Composer est requise. Consulter <<install-composer>> pour les instructions
  correspondantes.
...
Installeur d'un site de démo::
  Créez rapidement rapidement un site de démo temporaire qui utilise le serveur
  web intégré et la base de données SQLite qui sont pris en charge par PHP en
  exécutant la commande ci-dessous depuis le répertoire racine de votre projet.
  Dans ce cas, vous ne lancerez pas l'installeur interactif. Consulter la page
  https://www.drupal.org/docs/official_docs/evaluator-guide[Evaluator Guide] (en
  anglais) pour en apprendre plus.
