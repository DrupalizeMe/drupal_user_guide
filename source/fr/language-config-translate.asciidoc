...
_Régionalisation et langue_ > _Traduction de la configuration_
(_admin/config/regional/config-translation_).

. Localiser l'élément de configuration que vous souhaitez traduire. Par exemple,
pour traduire le nom du site, vous devez trouver _System information_. Pour la
configuration qui est groupée par type (par exemple, les vues ou les formats de
dates), cliquer sur le bouton _Liste_ pour voir toutes les configurations de ce
type, et localiser ensuite l'élément recherché.

. Cliquer sur _Traduire_ pour l'élement que vous avez localisé.
...
. Dans le menu d'administration _Gérer_, accéder à _configuration_ >
_Régionalisation et langue_ > _Traduction de la configuration_
(_admin/config/regional/config-translation_).

. Cliquer sur _Liste_ sur la ligne _Views_.

. Cliquer sur _Traduire_ sur la ligne Recettes.

. Cliquer sur _Ajouter_ dans la ligne _anglais_. La page _Ajouter une
traduction en anglais pour la vue Recettes_ apparaît.

. Rechercher _Affichages_ > _Paramètres de l'affichage de Défaut > _Options
d'affichage par défaut de Recettes_. Traduire le _Titre d'affichage_ de
"Recettes" en "Recipes".

. Rechercher _Affichages_ > _Paramètres d'affichage de Défaut > _Options
