...
[[glossary-display]] Affichage (dans une vue)::
  Type de rendu d'une <<glossary-view, vue>>, par exemple une page, un
  <<glossary-block, bloc>> ou un flue. Consulter <<views-parts>> pour plus
  d'informations.
(((Ajax,définition)))
[glossary]
[[glossary-ajax]] Ajax::
...
[[glossary-alias]] Alias::
   ...
   système assigne à une <<glossary-url,URL>> du site. Par exemple, vous
   pourriez assigner un alias _apropos_ à la page "À propos" de votre site, pour
   remplacer le chemin interne _/node/5_. Cela donnerait à la page l'URL
...
[[glossary-wizard]] Assistant::
   ...
   plus d'informations.
(((Authentifié,définition)))
[[glossary-authenticated]] Authentifié::
   Une personne (<<glossary-user,utilisateur>>) interagissant avec le site qui
   est connectée. Consulter <<user-concept>> pour plus d'informations.
(((Bloc,définition)))
[[glossary-block]] Bloc::
...
[[glossary-path]] Chemin::
   La dernière partie, unique, de l'<<glossary-url,URL>> interne que le système
   assigne à une page du site, qui peut être une page visible publiquement par
   les visiteurs ou une page d'administration. Par exemple, l'URL interne de la
   page À propos de votre site  pourrait être _http://example.com/node/5_, et
   dans ce cas, le chemin serait _node/5_. Consulter aussi
   <<glossary-alias,alias>>. Consulter <<content-paths>> pour plus
   d'informations.
(((Composer,définition)))
[[glossary-composer]] Composer::
   Le gestionnaire de dépendances de PHP utilisé par Drupal, <<glossary-drush,
   Drush>>, le _framework_ et d'autres. Il est le moyen d'installation à
   privilégier pour installer des projets Drupal. Consulter <<install-composer>>
...
(((Coder, définition)))
[[glossary-coder]] Conventions de codage::
   Les conventions de codage sont les règles destinées aux développeurs, qui
   définissent les bonnes pratiques, le formatage et autres, visant à ce que
   tout le monde utilise les mêmes conventions, et ait les mêmes attentes en
   visualisant du code. Consulter <<install-tools>> pour plus d'informations.
...
[[glossary-cron]] Cron::
   ...
   d'informations.
(((Cross Site Scripting (XSS), définition)))
[[glossary-cross-site-scripting]] Cross Site Scripting::
   Faille de sécurité souvent présente sur des sites web. Sur un site qui n'est
   pas bien protégé, des utilisateur mal intentionnés peuvent injecter un script
   dans des pages web qui sont vues par d'autres utilisateurs. Consulter
   <<structure-text-formats>> pour plus d'informations.
(((Dépôt, définition)))
[[glossary-repository]] Dépôt::
   Endroit où un système de gestion de version stocke tous les fichiers et
   répertoires d'un projet. Consulter <<install-tools>> pour plus
   d'informations.
(((Devel, définition)))
[[glossary-devel]] Devel::
   Module qui aide à la réalisation de tâches de développement comme le débogage
   ou l'inspection du code, l'analyse de requêtes, et la génération de contenu
   d'exemple. Consulter <<install-tools>> pour plus d'informations.
...
[[glossary-distribution]] Distribution::
   ...
   site ou créent même du contenu d'exemple à
   l'<<glossary-installing,installation>>. Consulter
...
(((Drupal Association (DA) ,définition)))
(((DA (Drupal Association),définition)))
[[glossary-drupal-association]] Drupal Association::
   Organisation à but non lucratif dont le but est de soutenir le projet Drupal
   et sa communauté. Consulter <<understanding-project>> pour plus
   d'informations.
(((Drush,définition)))
[[glossary-drush]] Drush::
  Interpréteur de commandes et interface permettant d'écrire des scripts pour
  Drupal. Consulter <<install-tools>> pour plus d'informations.
...
[[glossary-state]] État::
   ...
   pour plus d'informations.
(((Filtre, définition)))
[[glossary-filter]] Filtre (dans une vue)::
  Limite les données à afficher dans une <<glossary-view, vue>>, en se basant
  sur des critères tels que le statut de publication, le type de contenu ou la
  valeur d'un champ. Consulter <<views-parts>> pour plus d'informations.
(((Filtre contextuel, définition)))
[[glossary-contextual-filter]] Filtre contextuel (dans une vue)::
  Limite les données à afficher dans une <<glossary-view, vue>>, en se basant
  sur le contexte de l'<<glossary-display, affichage>> de la vue, comme l'URL
  complète de la page. Consulter <<views-parts>> pour plus d'informations.
...
(((Git, définition)))
[[glossary-git]] Git::
  <<glossary-vcs, Système de gestion de versions>> utilisé par les développeurs
  Drupal pour coordonner les modifications apportées par chacun. Git enregistre
  les changements qu'ils apportent à un projet donné dans une arborescence de
  répertoires appelé un <<glossary-repository, dépôt>> Git. Consulter
  <<install-tools>> pour plus d'informations.
...
[[glossary-installing]] Installation::
   Préparation du cœur de Drupal ou de n'importe quel module ou thème contribué
   en vue de son usage. Dans le cas du cœur de Drupal, cela consiste à
   télécharger les fichiers nécessaires, créer la base de données, configurer et
   exécuter le script d'installation. Consulter <<install-decide>> pour plus
   d'informations.
...
(((Processus éditorial, définition)))
[[glossary-editorial-workflow]] Processus éditorial::
   Processus de création, validation, modification et publication de contenu.
   Plusieurs personnes ayant différents rôles (par exemple, des rédacteurs de
   contenu ou des éditeurs) peuvent y prendre part. Consulter
   <<planning-workflow>> pour plus d'informations.
(((Réactif,définition)))
...
[[glossary-region]] Région::
   ...
   plus d'informations.
(((Relation, définition)))
[[glossary-relationship]] Relation (dans une vue)::
  Expansion de la donnée à afficher dans une vue, en mettant en relation le
  contenu de base avec d'autres entités de contenu. Consulter <<views-parts>>
  pour plus d'informations.
(((Révision,définition)))
...
[[glossary-session]] Session::
  Informations concernant les interactions d'un visiteur avec le site, par
  exemple s'il est connecté, ou ses cookies. Consulter <<understanding-data>>
  pour plus d'informations.
...
(((Site de développement,définition)))
[[glossary-development-site]] Site de développement::
   Copie du site de production qui est utilisée pour le développement, la mise à
   jour et les tests. Consulter <<install-dev-sites>> pour plus d'informations.
(((Site de pré-production,définition)))
[[glossary-staging-site]] Site de pré-production::
   Copie du site de production qui peut être utilisée pour tester, ou pour
   présenter des modifications au client pour approbation. Consulter
   <<install-dev-sites>> pour plus d'informations.
...
(((Système de gestion de versions, définition)))
[[glossary-vcs]] Système de gestion de versions::
   Logiciel qui conserve les copies des fichiers et l'historique de modification
   dans un <<glossary-repository, dépôt>>, et autorise l'ajout, la suppression
   et la mise à jour de fichiers. Consulter <<install-tools>> pour plus
   d'informations.
...
[[glossary-ui]] UI::
   ...
   (<<glossary-user-interface,interface utilisateur>>).
(((URL (Uniform Resource Locator),définition)))
(((Uniform Resource Locator (URL),définition)))
[[glossary-url]] URL::
   L'adresse unique d'une page web sur le Web, par exemple
   https://example.com/node/7. Consulter <<content-paths>> plus d'informations.
...
[[glossary-user-one]] Utilisateur un (Utilisateur 1)::
   ...
   l'<<glossary-installing,installation>> du site (et dont l'identifiant (_ID_) est
   1). Il a automatiquement tous les <<glossary-permission,droits>>, même si on
   ne lui a pas assigné de <<glossary-role,rôle>> d'administration. Consulter
...
[[glossary-workflow]] Workflow::
   See <<glossary-editorial-workflow, Processus éditorial>>.
(((XSS (Cross Site Scripting), définition)))
[[glossary-xss]] XSS::
   Acronyme pour <<glossary-cross-site-scripting, Cross Site Scripting>>.
