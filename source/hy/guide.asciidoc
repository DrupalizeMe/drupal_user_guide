...
// When this translation is done, make sure to update the scripts/lang-lc.conf (for your language code) at
// the same time so the chapter headings for Preface, Index, and Glossary in that file match what is in the source file for this issue.
