...
und und dieses verkleinert, quadratisch zugeschnitten oder schwarz-weiß anzeigen lassen (Sie können auch eine beliebige Kombination dieser Effekte verwenden). Das Kernsystem, stellt hierzu einen effizienten Weg zur Verfügung:
...
. Das so erzeugte Bild wird gesopeichert.
