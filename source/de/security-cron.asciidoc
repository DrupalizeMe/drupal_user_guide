...
Prüfen Sie, ob Cron-Wartungsaufgaben regelmäßig ausgeführt werden. Ist dies nicht der Fall,installieren Sie entweder das Modul Autmatic Cron oder lassen Sie Cron-Wartungsaufgaben außerhalb Ihrer Website ausführen.
...
oder oder mittels anderer Werkzeuge ausführen möchten. Das Modul Automated-Cron ist aus folgenden
...
_System_ > _Cron_ (_admin/config/system/cron_). Wählen Sie im Feld _Crona alle x-Miunten ausführen_ unter _Cron-Einstellungen_ das gewünschte Intervall aus und klicken Sie auf _Konfiguration Speichern_.
+
--
