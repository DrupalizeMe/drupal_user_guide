...
Wenn Sie den Composer verwenden möchten, muss er entweder innerhalb einer lokalen Entwicklungsumgebung direkt auf dem Server installiert sein, auf dem sich Ihre Live-Website befindent. Siehe: <<install-tools>>.
...
. Wechseln Sie auf der Kommandozeile in das Verzeichnis, dass sich eine Ebene über dem Verzeichnis befindet, in dem Sie
...
Kernsystems sin, ist es eine gute Idee, sie in die Datei _composer.json_ in
...
würden Sie nur 1.7 eingeben. Wenn Sie das Modul auf eien Version aktualisieren möchten, die kompatibel zu einer neueren Version des Hauptsystems z. b.  8.3.1 ist, würden Sie 8.3.1 eingeben. 
Geben Sie dann den folgenden Befehl im Stammverzeichnis Ihrer Website ein
(Ersetzen Sie den Systemnamen des Projekts +geofield+ sowie die korerekte Versionsnummer des gewünschten Projektes):
+
----
