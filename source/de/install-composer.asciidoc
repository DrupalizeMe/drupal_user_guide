...
Verwenden Sie den Composer, um Dateien und Abhängigkeiten des Kernsystems oder von installierten Zusatzmodulen und Themes herunterzuladen oder zu aktualisieren,
...
Wenn Sie den Composer verwenden möchten, muss er entweder innerhalb einer lokalen Entwicklungsumgebung direkt auf dem Server installiert sein, auf dem sich Ihre Live-Website befindet. Siehe: <<install-tools>>.
...
. Wechseln Sie auf der Kommandozeile in das Verzeichnis, dass sich eine Ebene über dem Verzeichnis befindet, in das Sie
...
Abhängigkeiten. Für Sites, die mit Version 8.8.0 oder höher des Kernsystems erstellt wurden
...
Hierfür führen Sie für jedes heruntergeladene Modul, Theme oder Profil einmal den folgenden Befehl aus
wobei der Name des gewünschten Projekts durch +systemname+ ersetzt wird, um das jeweilige Projekt zur Datei _composer.json_
hinzufügen:
...
. Wenn Sie eine bestimmte Version benötigen, legen Sie über die Versionsnummer fest, welche Version Sie installieren möchten. Zur Installation der  Version 8.x-1.7 eines von der Community bereitgestellten Zusatzmoduls
würden Sie nur 1.7 eingeben. Wenn Sie das Modul auf eiene Version aktualisieren möchten, die kompatibel zu einer neueren Version des Hauptsystems z. b.  8.3.1 ist, würden Sie 8.3.1 eingeben. 
Geben Sie dann den folgenden Befehl im Stammverzeichnis Ihrer Website ein
(Ersetzen Sie den Systemnamen des Projekts +geofield+ sowie die korrerekte Versionsnummer des gewünschten Projektes):
+
----
