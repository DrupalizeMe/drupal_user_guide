...
Drupal-Website bereitstellt. Eine neue Funktion, können Sie durch die _Installation_ eines zusätzlichen Moduls aktivieren Durch die _De-Installation_ eines Moduls, kann eine bestimmte Funktion auf Wunsch auch abgeschaltet werden. Vor der Deinstallation müssen Sie möglicherweise Daten und Konfiguration entfernen die mit der Funktionsweise eines Moduls in Zusammenhang stehehen. Jedes Modul, das installiert wird, erhöht die Ladezeit der Website. Daher ist es eine gute Idee, Module die nicht benötigt werden, zu deinstallieren, um die Ladezeit einer Website so kurz wie möglich zu halten.
