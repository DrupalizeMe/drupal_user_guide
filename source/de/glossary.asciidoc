...
[[glossary-content-type]] Inhaltstyp::
   ...
   Inhaltstyp für statische Seiten und einen Wieteren Inhaltstyp enthalten, mit dem Lieferantenlisten erzeugt werden können.
   Siehe <<planning-data-types>> für weitere Informationen.
...
[[glossary-contextual-link]] Kontextlink::
   ...
   Website, die in dem Kontext angezeigt wird, in dem diese Funktion mit dem jeweiligen Element auf einer Website 
   in  Zusammenhang steht. Beispiel: Ein
   Link zur Konfiguration eines <<glossary-menu,Menüs>>, der angezeigt wird, wenn Sie den Mauszeiger über
...
[[glossary-distribution]] Distribution::
   Ein einzelner Download, der eine schnelle Lösung für die Einrichtung einer Webiste enthält, die einen spezifischen Anwendungsfall abdeckt.
...
[[glossary-entity]] Entität::
   Ein Element, das entwerder  dem Typ <<glossary-content,Inhalt>> oder
   <<glossary-configuration,Konfiguration>> entsprechen kann. Der Begriff  wird im allgemeinen bestimmte Arten von Ihhalten auf einer Website verwendet. Beispiele sind:
...
[[glossary-entity-subtype]] Entitätssubtyp::
   ...
   <<glossary-content-item,Inhalt>> beschrieben, dass die auf der Wochenmarkt-Website verwendeten Entitätstypen Entitätssubtypen des Entitätstyps <<glossary-content-type,Inhaltstypen>>) sind. Ein Subtyp für statische Seiten und ein Subtyp für Lieferanten.
   Jeder Subtyp enthält dabei seine eigenen Felder. Auch der Begriff _Bündel/Bundle_ wird als Bezeichung solcher Elemente verwendet
