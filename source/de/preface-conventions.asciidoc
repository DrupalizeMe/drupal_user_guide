...
Inhalten Sie sich auseinandergesetzt haben müssen, um das Thema, das Sie
...
die Sie auf Ihrer Website bereits abgeschlossen haben müssen, damit die
Voraussetzungen erfüllt sind, die erfüllt sein müssen, damit Sie eine Aufgabe,
die in dem Thema behandelt wird, das sie gerade durcharbeiten möchten, umsetzen
können.
...
und dann auf den Menüpunkt _Struktur_ und den untergeordneten Menüpunkt _Taxonomie_
klicken, so dass Sie am Ende auf einer Seite mit der
...
_System_ > _Grundeinstellungen der Website_ (_admin/config/system/site-information_) navigieren würden:
...
weitere Informationen). Beachten Sie auch, dass Sie auf einigen Formularen
